...
class Technieum:
    """Main Technieum orchestrator"""

    def __init__(self, targets: List[str], output_dir: str = "output",
                 db_path: str = "technieum.db", threads: int = 5,
        ...
        _env_continue = os.getenv("TECHNIEUM_CONTINUE_ON_FAIL", "1").lower() in ("1", "true", "yes")
        self.continue_on_fail = (not strict_mode) and _env_continue
        self.resume = resume
        self.phase_timeouts = {
            1: int(os.getenv("TECHNIEUM_PHASE1_TIMEOUT", str(self.phase_timeout_default))),
            2: int(os.getenv("TECHNIEUM_PHASE2_TIMEOUT", str(max(self.phase_timeout_default, 7200)))),
            3: int(os.getenv("TECHNIEUM_PHASE3_TIMEOUT", str(max(self.phase_timeout_default, 10800)))),
            4: int(os.getenv("TECHNIEUM_PHASE4_TIMEOUT", str(max(self.phase_timeout_default, 14400)))),
            5: int(os.getenv("TECHNIEUM_PHASE5_TIMEOUT", str(max(self.phase_timeout_default, 3600)))),
            6: int(os.getenv("TECHNIEUM_PHASE6_TIMEOUT", str(max(self.phase_timeout_default, 3600)))),
            7: int(os.getenv("TECHNIEUM_PHASE7_TIMEOUT", str(max(self.phase_timeout_default, 1800)))),
            8: int(os.getenv("TECHNIEUM_PHASE8_TIMEOUT", str(max(self.phase_timeout_default, 1800)))),
            9: int(os.getenv("TECHNIEUM_PHASE9_TIMEOUT", str(max(self.phase_timeout_default, 1800)))),
        }
...
"""
    ...
    def run(self, phases: List[int] = None):
        ...
        self.banner()
        self.logger.info(f"Starting Technieum for {len(self.targets)} target(s)")
...
Phases:
  ...
  4 - Vulnerability Scanning
    ...
    # Default output_dir: config.yaml general.output_dir → env TECHNIEUM_OUTPUT_DIR → 'output'
    ...
    parser.add_argument('-d', '--database', default=os.getenv('TECHNIEUM_DB_PATH', 'technieum.db'), help='Database file path (default: technieum.db or TECHNIEUM_DB_PATH)')
